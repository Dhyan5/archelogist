# Database Schema Architecture: Software Archaeologist

The application uses MySQL 8.0 for relational persistence of users, scans, code metadata, dependency graphs, risk metrics, and technical debt findings.
...
```
[ users ] 1 --- N [ repositories ] 1 --- N [ scans ]
                                             |
   +-----------------+-----------------------+------------------+-------------------+
   |                 |                       |                  |                   |
1--N              1--N                    1--N               1--N                1--N
[ file_nodes ]   [ dependencies ]    [ api_endpoints ]  [ tech_debt_items ] [ git_hotspots ]
   |
1--N
[ code_symbols ]
```
...
CREATE TABLE scans (
    ...
    architecture_confidence INT DEFAULT 0,
